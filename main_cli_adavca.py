...
def parse_args():
    ...
    parser.add_argument('--n-anneal', type=int, default=20,
                       help='Maximum number of annealing steps (outer iterations)')
    parser.add_argument('--n-train', type=int, default=5,
                       help='Number of training steps per annealing step')
    parser.add_argument('--T0', type=float, default=10.0,
                       help='Initial temperature')
    # adaptive schedule hyperparameters
    parser.add_argument('--T-min', type=float, default=1e-3,
                       help='Minimum temperature to stop adaptive annealing')
    parser.add_argument('--delta-kl', type=float, default=1e-2,
                       help='Target KL step size for adaptive temperature updates')
    parser.add_argument('--w-clip', type=float, default=1e6,
                       help='Max importance weight for SNIS in adaptive schedule')
    # paths
...
if __name__ == "__main__":
    ...
    # use adaptive variational configurational annealing (adavca)
    model = adavca(
        N=N,
        protein=protein,
