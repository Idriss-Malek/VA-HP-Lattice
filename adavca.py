...
"""
adaVCA Class (adaptive temperature schedule)
"""
class adavca:

    def __init__(self, N, protein, n_warmup, n_anneal, n_train, T0, T_min, delta_kl, w_clip, ckpt_path, seed=111):
        """
        N - len(protein) - 1
        protein - HP sequence; list of int
        n_warmup - number of warmup iterations
        n_anneal - maximum number of annealing steps (outer iterations)
        n_train - number of gradient/training steps for each annealing step
        T0 - initial temperature
        T_min - minimum temperature (stopping threshold)
        delta_kl - target KL step size between successive π_T distributions
        w_clip - max importance weight for SNIS
        """
    ...
    def test_run(self):
        ...
        with tf.compat.v1.variable_scope(self.DRNN.scope,reuse=tf.compat.v1.AUTO_REUSE):
            with self.DRNN.graph.as_default():
                ...
                # ANNEALING loop - core training with annealed/decaying temperature (adaptive schedule)
                ...
                # adaptive schedule constants
                eps_E = 1e-12
                eps_zeta = 1e-12

                for it0 in range(self.num_anneal):
                    # optional early stopping on T_min
                    if T <= self.T_min:
                        break

                    # ===========================
                    # Adaptive temperature update (KL-controlled, FO+SO)
                    # ===========================
                    # estimation batch at current T
                    est_samples, est_log_probs, est_positions = sess.run(outputs)
                    est_validity = np.zeros(self.numsamples, dtype=np.int64)
                    for i in range(est_positions.shape[0]):
                        est_validity[i] = fold_is_valid(est_positions[i])
                    est_grids = create_fold_grid(est_positions, self.protein)
                    est_energies = energyv2(est_grids, est_positions, self.protein)
                    est_energies = est_energies * est_validity + penalty[est_validity]

                    mean_E_p = np.mean(est_energies)
                    min_E_p = np.min(est_energies)
                    max_E_p = np.max(est_energies)
                    var_E_p = np.var(est_energies)

                    # SNIS weights for pi_T
                    beta = 1.0 / max(T, 1e-12)
                    logw = (-beta * est_energies.astype(np.float64) - est_log_probs.astype(np.float64))
                    logw = logw - np.max(logw)
                    w = np.exp(logw)
                    if np.isfinite(self.w_clip):
                        w = np.minimum(w, self.w_clip)
                    sum_w = np.sum(w) + 1e-20
                    w_norm = w / sum_w
                    ess = (sum_w ** 2) / (np.sum(w ** 2) + 1e-20)

                    E_pi_H_hat = np.sum(w_norm * est_energies.astype(np.float64))
                    Var_pi_H_hat = np.sum(w_norm * (est_energies.astype(np.float64) - E_pi_H_hat) ** 2)
                    DeltaE_hat = max(mean_E_p - E_pi_H_hat, 0.0)
                    zeta_hat = Var_pi_H_hat / max(T ** 4, 1e-24)

                    # first-order and second-order ΔT proposals
                    if DeltaE_hat <= eps_E:
                        DeltaT_FO = -np.inf
                    else:
                        DeltaT_FO = - self.delta_kl * (T ** 2) / (DeltaE_hat + 1e-20)

                    if zeta_hat <= eps_zeta:
                        DeltaT_SO = -np.inf
                    else:
                        DeltaT_SO = - np.sqrt(2.0 * self.delta_kl / (zeta_hat + 1e-20))

                    # use FO+SO (max) as in default adaVCA schedule_mode="fo+so"
                    chosen_DeltaT = max(DeltaT_FO, DeltaT_SO)

                    # safety checks (fallback small negative step if needed)
                    if (not np.isfinite(chosen_DeltaT)) or (chosen_DeltaT > 0.0):
                        chosen_DeltaT = -min(1e-3, 0.05 * max(T, 1e-6))

                    # enforce T_min
                    if T + chosen_DeltaT < self.T_min:
                        if T > 5 * self.T_min:
                            chosen_DeltaT = -self.T_min
                        else:
                            chosen_DeltaT = self.T_min - T

                    # update T and record schedule
                    T = T + chosen_DeltaT
                    temperatures[it0] = T

                    # ===========================
                    # training loop - perform self.num_train gradient steps at this T
                    # ===========================
